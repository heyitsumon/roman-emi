...
class PurchaseController extends Controller
{
    ...
    {
        $purchases = Purchase::with(['customer.location', 'product', 'model'])->orderBy('created_at', 'desc')->paginate(100);
        $totalPurchases = Purchase::count();
        $totalNet = Purchase::sum('net_price');
        $totalDown = Purchase::sum('down_price');

        return view('purchases.index', compact('purchases', 'totalPurchases', 'totalNet', 'totalDown'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Split the remaining balance in cents so every installment sums exactly.
            $totalDueCents = (int) round(($purchase->net_price - $purchase->down_price) * 100);
            $baseEmiCents = intdiv($totalDueCents, $purchase->emi_plan);
            $remainderCents = $totalDueCents % $purchase->emi_plan;

            // Create installments
            $installments = [];
            for ($i = 0; $i < $purchase->emi_plan; $i++) {
                $installmentCents = $baseEmiCents + ($i < $remainderCents ? 1 : 0);
